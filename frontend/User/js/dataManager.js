...
class DataManager {
    ...
                
                // Load expenses from API (PRIORITY: API data, but never wipe local cache with empty list)
    ...
                    const apiItems = expensesResult.data.items;
                    if (Array.isArray(apiItems) && apiItems.length > 0) {
                        // Overwrite only when server has data
                        this.data.expenses = apiItems;
                        console.log(`✅ Loaded ${this.data.expenses.length} expenses from API`);
                    } else {
                        // Keep local cache to avoid "data loss" feeling
                        console.warn('API returned 0 expenses; keeping local cache and marking for sync');
                        this.markNeedsSync();
                    }
    ...
                }
                
                // Load categories from API (PRIORITY: API data, but never wipe local cache with empty list)
    ...
                    const apiCats = categoriesResult.data.items;
                    if (Array.isArray(apiCats) && apiCats.length > 0) {
                        this.data.categories = apiCats;
                        console.log(`✅ Loaded ${this.data.categories.length} categories from API`);
                    } else {
                        console.warn('API returned 0 categories; keeping local cache and marking for sync');
                        this.markNeedsSync();
                    }
