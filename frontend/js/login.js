...
    if (!form.__LOGIN_SUBMIT_BOUND) {
        form.addEventListener('submit', handleLogin);
        form.__LOGIN_SUBMIT_BOUND = true;
    }
}
...
                monthly_budget: (userData.monthly_budget !== undefined && userData.monthly_budget !== null)
                    ? userData.monthly_budget
                    : ((userData.balance !== undefined && userData.balance !== null) ? userData.balance : 0)
            };

            const saveProfileResult = await window.apiRequest('/api/profile', {
                method: 'PUT',
                body: JSON.stringify(profilePayload)
            });

            if (saveProfileResult && saveProfileResult.ok && saveProfileResult.data) {
                const saved = saveProfileResult.data.profile || saveProfileResult.data;
                // Merge back any canonical values from server
                if (saved.balance !== undefined) userData.balance = saved.balance;
                if (saved.gender !== undefined) userData.gender = saved.gender;
                if (saved.currency) userData.currency = saved.currency;
                if (saved.phone !== undefined) userData.phone = saved.phone;
                if (saved.monthly_budget !== undefined) userData.monthly_budget = saved.monthly_budget;
            }
        } catch (e) {
            console.warn('Không thể lưu hồ sơ vào database ngay sau đăng nhập. Sẽ tiếp tục với localStorage.', e);
        }
    }
...
    try {
        if (userData.monthly_budget !== undefined && userData.monthly_budget !== null) {
            localStorage.setItem('monthly_budget', String(userData.monthly_budget));
        }
    } catch (e) {
        console.warn('Không thể lưu monthly_budget vào localStorage:', e);
    }
...
    // Notify other pages/components that profile has been updated
    try {
...
                // Backend trả về trực tiếp user object (không bọc trong { user })
                const verifiedUser = verifyResult && verifyResult.ok
                    ? (verifyResult.data && (verifyResult.data.user || verifyResult.data))
                    : null;
                if (verifiedUser) {
...
    // Re-run API base discovery on DOM ready to ensure latest override/localStorage is respected
    (async () => {
        try {
            if (typeof window.tryDiscoverApiBase === 'function') {
                await window.tryDiscoverApiBase();
            }
            console.log('🌐 API Base đang sử dụng:', window.API_BASE || getApiBase());
        } catch (e) {
            console.warn('Không thể khám phá API Base ở DOMContentLoaded:', e);
        }
    })();
...
        checkExistingLogin();
    }, 500);
    // Đảm bảo chỉ init một lần
    if (!window.__LOGIN_INIT_DONE) {
        window.__LOGIN_INIT_DONE = true;
        initSimpleLogin();
    }
});
